...
function keysCheck() {
    if(!key[87] && key[83] || key[87] && !key[83] || !key[38] && key[40] || key[38] && !key[40]) {
        camF = mulV(cam.D,cam.M);
        if(key[87] || key[38]) cam.P = vSubV(cam.P,camF);
        if(key[83] || key[40]) cam.P = vAddV(cam.P,camF);
    }
    if(!key[65] && key[68] || key[65] && !key[68] || !key[37] && key[39] || key[37] && !key[39]) {
        let camX = crossProd( cam.D, cam.U);
            camX = mulV( normal(camX), cam.M);
        if(key[65] || key[37]) cam.P = vAddV(cam.P,camX);
        if(key[68] || key[39]) cam.P = vSubV(cam.P,camX);
    }
